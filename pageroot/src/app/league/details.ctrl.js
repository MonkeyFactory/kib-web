...
angular.module('kibApp').controller('LeagueDetailsPageController', function($scope, $window, $stateParams, kibservice){
	...
	$scope.league.$promise.then(function(){
		...
		$scope.leagueActive = (moment() >= moment($scope.league.StartDate) && (!$scope.league.EndDate || moment() < moment($scope.league.EndDate).add(1, 'd') ));
	});
	...
	$scope.scorehistory.$promise.then(function(){
		...
		//Add players (columns)
		for(var i=1;i < $scope.scorehistory.length;i++){
		...
		for(i=1;i < $scope.scorehistory[0][0] + 1;i++){
			...
			//Add player scores
			for(var x=1;x < $scope.scorehistory.length;x++){
